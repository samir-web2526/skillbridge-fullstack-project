...
type TutorProfileWhereInput = Prisma.TutorProfileWhereInput;
...
const getTutor = async (payload: {
  ...
  const formattedTutors = tutors.map((tutor:any) => {
    ...
    const averageRating =
      totalReview > 0
        ? Number(
            tutor.review.reduce((acc: number, r:any) => acc + r.rating, 0) /
              totalReview,
          )
        : 0;
...
const getTutorById = async (tutorId: string) => {
  ...
  const averageRating =
    totalReview > 0
      ? Number(tutor.review.reduce((acc: number, r) => acc + r.rating, 0) / totalReview)
      : 0;
...
const deleteTutor = async (tutorId: string) => {
  ...
  if (existBooking) {
    throw new Error("You cannot delete tutor with existing bookings");
  }
